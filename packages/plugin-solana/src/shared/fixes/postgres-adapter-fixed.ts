...
type QueryConfig<I> = pgPkg.QueryConfig<I>;
type QueryConfigValues<I> = pgPkg.QueryConfigValues<I>;
type QueryResult<R> = pgPkg.QueryResult<R>;
...
import { convertVector } from "./vector-fix.js";
    ...
    implements IPostgresDatabaseAdapter
{
    ...
    async createMemory(memory: Memory, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            // Get the correct type value - use memory.type for wallet registrations
            const typeField = memory.content?.type === "wallet_registration" || 
                              memory.content?.type === "pending_wallet_registration" 
                              ? (memory as any).type || memory.content.type  // Use memory.type if present, fallback to content.type 
                              : tableName;  // Default to tableName for non-wallet registrations
            ...
            elizaLogger.debug("PostgresAdapter createMemory:", {
                ...
                memoryType: (memory as any).type,
                usingTypeField: typeField,
                isWalletRegistration: memory.content?.type === "wallet_registration" || memory.content?.type === "pending_wallet_registration"
            });
            ...
            await this.pool.query(
                ...
                [
                    memory.id ?? v4(),
                    typeField, // Use the correct type field value
    ...
    async getMemories(params: {
        ...
        // Modified check to:
        // 1. Allow missing roomId when embeddings are disabled
        // 2. Always allow the conversation room ID "00000000-0000-0000-0000-000000000001" even if it's falsy
        if (!params.roomId) {
            ...
            // Special case: Set roomId to the conversation room ID if it's missing
            // This allows wallet registration queries to succeed
            elizaLogger.info("Setting missing roomId to conversation room ID: 00000000-0000-0000-0000-000000000001");
            params.roomId = "00000000-0000-0000-0000-000000000001" as UUID;
    ...
    async getCachedEmbeddings(opts: {
        ...
        return this.withDatabase(async () => {
            try {
                // Truncate query input to 255 chars to meet PostgreSQL levenshtein limit
                const truncatedInput = opts.query_input.slice(0, 255);
                
                // Log if truncation happened
                if (truncatedInput.length < opts.query_input.length) {
                    elizaLogger.debug("Truncated levenshtein input text:", {
                        originalLength: opts.query_input.length,
                ...
                elizaLogger.debug("Fetching cached embeddings:", {
                    ...
                    threshold: opts.query_threshold
                });

                // Improved SQL query to calculate levenshtein only once, eliminating redundant calls
                const sql = `
                    WITH content_text AS (
                        SELECT
                            id,
                            embedding,
                            LEFT(COALESCE(content->>$2, ''), 255) as content_text
                        FROM memories
                        WHERE type = $3
                    ...
                    scored_content AS (
                        SELECT
                            id,
                            embedding,
                            levenshtein($1, content_text) as levenshtein_score
                        FROM content_text
                    )
                    SELECT
                        embedding,
                ...
                const { rows } = await this.pool.query(sql, [
                    truncatedInput, // Using truncated input instead of original
                ...
                elizaLogger.error("Error in getCachedEmbeddings:", {
                    ...
                    truncatedLength: opts.query_input.slice(0, 255).length
                });
                throw error;
            }
        }, "getCachedEmbeddings");
    ...
    async createMemoryWithTransaction(memory: Memory, tableName: string, transaction: PoolClient): Promise<void> {
        const query = {
            text: `INSERT INTO ${tableName} (id, content, room_id, user_id, agent_id) VALUES ($1, $2, $3, $4, $5)`,
            values: [memory.id, memory.content, memory.roomId, memory.userId, memory.agentId]
        };
        await transaction.query(query);
    }

    async removeMemoryWithTransaction(id: UUID, tableName: string, transaction: PoolClient): Promise<void> {
        const query = {
