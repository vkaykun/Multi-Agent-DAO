import { IAgentRuntime, Memory, UUID, Action } from "@elizaos/core";
import { BaseContent, ContentStatus, AgentType, AgentTypes } from "../types/base";
import { elizaLogger } from "@elizaos/core";

export interface ActionDefinition<T extends BaseContent = BaseContent> {
    type: string;
    category: AgentType;
    handler: (content: T, runtime: IAgentRuntime) => Promise<boolean>;
    validate: (content: T, runtime: IAgentRuntime) => Promise<boolean>;
    ...
    public registerAction(action: ActionDefinition): void {
        ...
        const runtimeAction: Action = {
            ...
            handler: async (runtime: IAgentRuntime, message: Memory) => {
                const content = message.content as BaseContent;
                return action.handler(content, runtime);
            },
            validate: async (runtime: IAgentRuntime, message: Memory) => {
                const content = message.content as BaseContent;
                return action.validate(content, runtime);
            }
        };
