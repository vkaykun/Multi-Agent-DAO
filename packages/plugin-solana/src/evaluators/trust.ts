...
class TrustScoreManager {
    ...
    async createTradePerformance(contractAddress: string, userId: string, options: { buy_amount: number, is_simulation: boolean }) {
        // Implementation can be added later
        elizaLogger.debug("Creating trade performance", { contractAddress, userId, options, provider: this.tokenProvider });
    }
}
...
async function handler(runtime: IAgentRuntime, message: Memory) {
    ...
    for (const rec of filteredRecommendations) {
        ...
        if (!publicKey) {
            elizaLogger.warn("No public key available");
            continue;
        }
        ...
        if (!account) {
            elizaLogger.warn("Could not find account for user");
            continue;
        }
        const userId = account.id;

        if (!agentId) {
            elizaLogger.warn("No agent ID available");
            continue;
        }
