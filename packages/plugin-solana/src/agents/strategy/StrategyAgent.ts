...
import {
    StrategyType,
    StrategyCondition,
    StrategyContent as ImportedStrategyContent,
    StrategyExecution,
    PositionUpdate,
...
import { MemoryWithUniqueKeys } from "../../shared/memory/BaseMemoryManager";
import { getUserIdFromMessage } from "../../shared/utils/userUtils.ts";

// Local interfaces for memory content types
interface LocalStrategyContent extends BaseContent {
    type: "strategy";
    strategyId: UUID;
    token: string;
    baseToken: string;
...
interface LocalPositionContent extends BaseContent {
...
interface LocalStrategyExecutionContent extends BaseContent {
...
interface StrategyContent extends BaseContent {
    type: "strategy";
    strategyId: UUID;
    token: string;
    status: ContentStatus;
...
interface PositionContent extends BaseContent {
    type: "position_update";
    positionId: UUID;
    strategyId: UUID;
    token: string;
...
interface StrategyExecutionContent extends BaseContent {
    type: "strategy_execution_request";
    strategyId: UUID;
    executionId: UUID;
    token: string;
...
export class StrategyAgent extends BaseAgent {
    ...
    public async initialize(): Promise<void> {
        ...
        this.subscribeToMemory("strategy_execution", async (memory) => {
            const content = memory.content as ImportedStrategyContent;
            if (!content.id || !content.strategyType || !content.baseToken || !content.conditions) {
                elizaLogger.error("Invalid strategy execution content: missing required fields");
                return;
            }
        ...
        this.monitoringInterval = setInterval(async () => {
            ...
            try {
                ...
                for (const strategy of activeStrategies) {
                    ...
                    try {
                        await this.checkStrategyConditions(strategy.content as ImportedStrategyContent);
                    } finally {
    ...
    private async checkStrategyConditions(strategy: ImportedStrategyContent): Promise<void> {
        try {
            const priceInfo = await this.swapService.getTokenPrice(strategy.token);
            if (!priceInfo || priceInfo.error) {
                elizaLogger.error("Could not get price for token:", strategy.token);
                return;
            }

            const currentPrice = priceInfo.price;

            if (Array.isArray(strategy.conditions)) {
    ...
    private async executeStrategyAction(strategy: ImportedStrategyContent, condition: StrategyCondition): Promise<void> {
        const memoryManager = this.runtime.messageManager as IMemoryManager;
        ...
        try {
            ...
            const request: SwapRequest = {
                ...
                toToken: strategy.baseToken,
                amount: amount.toString(),
                reason: "strategy_triggered" as const,
                requestId: requestId,
    ...
    protected async handleMemory(memory: Memory): Promise<void> {
        ...
        try {
            // First check for message type since it needs special handling
            ...
            switch (content.type) {
                ...
                case "strategy_status_changed":
                    elizaLogger.info(`Strategy status changed: ${content.text}`);
                ...
                case "position_update":
                    await this.handlePositionUpdate(memory);
                    break;
                    
                case "price_update":
                    await this.handlePriceUpdate(memory);
                    break;
                    
                // Proposal events
                case "proposal_passed":
                case "proposal_status_changed":
                    // Check if this is a passed proposal that needs handling
                    if (content.type === "proposal_passed" || 
                        (content as any).status === "pending_execution" || 
                        (content as any).status === "passed") {
                ...
                case "swap_execution_result":
                    await this.handleSwapCompleted(memory);
                    break;
                    
                default:
                    elizaLogger.debug(`Unhandled memory type in StrategyAgent: ${content.type}`);
            }
        } catch (error) {
            elizaLogger.error(`Error handling memory type ${content.type}:`, error);
        }
    }
    ...
    private async handleStrategyEvent(content: Content): Promise<void> {
        if (this.isValidStrategyContent(content)) {
            elizaLogger.debug("Handling strategy event:", content);
            // Process strategy event
        } else {
            elizaLogger.warn("Invalid strategy content received:", content);
        }
    }
        ...
        try {
            ...
            await this.sendMessage({
                ...
                content: {
                    ...
                    id: stringToUuid(`suggestion-${Date.now()}`),
                    text: message,
    ...
    protected async executeWithValidation<T extends Record<string, unknown>, R>(
        ...
        try {
            ...
            elizaLogger.error("Error executing with validation:", error);
            throw error;
        }
    }
     ...
     * @param message The message containing user identification
     * @returns UUID formatted user ID
     */
    private getUserIdFromMessage(message: AgentMessage): UUID {
        return getUserIdFromMessage(message);
    }

    // Restore loadActions method
    ...
    private async handlePositionUpdate(memory: Memory): Promise<void> {
        try {
            const content = memory.content as PositionContent;
            elizaLogger.debug("Handling position update:", content);
            
            // Forward to StrategyExecutor
            if (content.positionId && content.strategyId) {
                await this.strategyExecutor.updatePositionAmount(
                    content.positionId,
                    parseFloat(content.amount),
                    parseFloat(content.entryPrice),
                    0, // Default to 0 if soldAmount not provided
                    'take_profit' // Use valid value instead of 'position_update'
                );
            }
        } catch (error) {
            elizaLogger.error("Error handling position update:", error);
        }
    }

    private async handlePriceUpdate(memory: Memory): Promise<void> {
        try {
            const content = memory.content;
            elizaLogger.debug("Handling price update:", content);
            
            // Check for active strategies that might need to be evaluated based on new price
            const strategies = await this.runtime.messageManager.getMemories({
                roomId: ROOM_IDS.STRATEGY,
                count: 100
            });
            ...
            const activeStrategies = strategies.filter(mem => 
                mem.content.type === "strategy" && 
                mem.content.status === "active" &&
                mem.content.token === content.token // Only check strategies for this token
            );
            
            // Process each affected strategy
            for (const strategy of activeStrategies) {
                // Get a lock specific to this strategy
                const strategyLock = await this.acquireDistributedLock(`strategy-${strategy.content.id}`);
                if (!strategyLock) continue;
            ...
            elizaLogger.error("Error handling price update:", error);
        }
    }

    /**
     * Override setupMemorySubscriptions to ensure we subscribe to all required memory types
     * and maintain backward compatibility with legacy memory types
     */
    protected override setupMemorySubscriptions(): void {
        // First call the parent method to set up standard subscriptions from registry
        super.setupMemorySubscriptions();
        
        // Explicitly subscribe to backwards compatibility types
        this.subscribeToMemory("proposal_passed", async (memory: Memory) => {
            await this.handleMemory(memory);
        });
        
        this.subscribeToMemory("proposal_executed", async (memory: Memory) => {
            await this.handleMemory(memory);
        });
        
        this.subscribeToMemory("swap_completed", async (memory: Memory) => {
            await this.handleMemory(memory);
        });
        
        elizaLogger.info("StrategyAgent: Additional memory subscriptions configured successfully");
    }
}