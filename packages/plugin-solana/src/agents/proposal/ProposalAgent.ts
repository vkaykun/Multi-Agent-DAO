...
export class ProposalAgent extends BaseAgent {
    ...
    protected async validateAction(content: BaseContent): Promise<boolean> {
        ...
        switch (content.type) {
            case "proposal":
            case "proposal_created": // Add support for proposal_created type
    ...
    protected async handleMemory(memory: Memory): Promise<void> {
        ...
        switch (content.type) {
            case "proposal":
            case "proposal_created": // Add explicit handling for proposal_created type
    ...
    private async handleProposeCommand(message: ExtendedAgentMessage): Promise<void> {
        ...
        try {
            await withTransaction(memoryManager as unknown as TransactionManager, async () => {
                ...
                // Create proposal memory - update to use both types for compatibility
                const proposalMemory: Memory = {
                    ...
                    content: {
                        ...proposal,
                        // Add both types for maximum compatibility
                        type: "proposal", // Keep for backward compatibility
                        updatedAt: Date.now()
                    },
                    roomId: ROOM_IDS.DAO,
                    createdAt: Date.now()
                ...
                await memoryManager.createMemory(proposalMemory);

                // Create a proposal_created memory for compatibility with registry
                await memoryManager.createMemory({
                    id: stringToUuid(`proposal-created-${proposal.id}`),
                    userId: this.runtime.agentId,
                    agentId: this.runtime.agentId,
                    content: {
                        ...proposal,
                        type: "proposal_created", // Use the type from registry
                        updatedAt: Date.now()
                    },
                    roomId: ROOM_IDS.DAO,
                    createdAt: Date.now()
    ...
    private async processVote(
        ...
        return this.withTransaction('processVote', async () => {
            ...
            const proposal = await this.runtime.messageManager.getMemoryWithLock(proposalUuid);
            if (!proposal) {
            ...
            if (!this.isValidProposalContent(content)) {
                return { success: false, error: 'Invalid proposal content' };
            }

            const newVotes = isYesVote ? 
                { yes: [...content.yes, userUuid] } : 
                { no: [...content.no, userUuid] };

            const success = await this.runtime.messageManager.updateMemoryWithVersion(
                proposalUuid,
                {
                    content: {
                        ...content,
            ...
            if (!success) {
                return { success: false, error: 'Concurrent vote detected - please retry' };
            }

            return {
                success: true,
                yesCount: content.yes.length + (isYesVote ? 1 : 0),
                noCount: content.no.length + (isYesVote ? 0 : 1)
            };
        });
    }
    ...
    private async updateProposalStatus(
        ...
        return this.withTransaction('updateProposalStatus', async () => {
            const proposal = await this.runtime.messageManager.getMemoryWithLock(proposalId);
            if (!proposal) {
                return false;
            }
            ...
            if (!success) {
                throw new Error('Concurrent status update detected - please retry');
            }
    ...
    async createProposal(proposal: ProposalContent): Promise<void> {
        const shortId = proposal.id.split('-')[0];
        
        // Create proposal memory (backward compatibility)
        await this.runtime.messageManager.createMemory({
            id: stringToUuid(`proposal-${shortId}`),
            content: proposal,
            roomId: ROOM_IDS.DAO,
        ...
        } as MemoryWithUniqueKeys);
        
        // Create proposal_created memory for registry compatibility
        await this.runtime.messageManager.createMemory({
            id: stringToUuid(`proposal-created-${shortId}`),
            content: {
                ...proposal,
                type: 'proposal_created'
            },
     ...
     * and maintain backward compatibility with both "proposal" and "proposal_created"
     */
    protected override setupMemorySubscriptions(): void {
        ...
        // Explicitly subscribe to proposal for backward compatibility
        this.subscribeToMemory("proposal", async (memory: Memory) => {
            await this.handleMemory(memory);
        });
        
        // Subscribe to proposal_monitoring which was missing from the registry
        this.subscribeToMemory("proposal_monitoring", async (memory: Memory) => {
            await this.handleMemory(memory);
        });
        
        elizaLogger.info("ProposalAgent: Additional memory subscriptions configured successfully");
    }

    async createMemoryWithFallback(memory, unique = false) {
      try {
        // Ensure content is properly populated before creating memory
        if (!memory.content || Object.keys(memory.content).length === 0) {
          elizaLogger.error("Cannot create memory with empty content");
          throw new Error("Cannot create memory with empty content");
        }
        
        // Make sure memory has required fields
        if (!memory.id) {
          memory.id = stringToUuid(crypto.randomUUID());
        }
        
        // Try the original method
        return await this.runtime.messageManager.createMemory(memory, unique);
      } catch (error) {
        elizaLogger.warn(`Memory creation error for ${memory?.id}: ${error.message}`);
        
        // Only retry if content is valid
        if (memory?.content && Object.keys(memory.content).length > 0) {
          try {
            // Set skipEmbedding flag to avoid embedding issues
            if (!memory.metadata) {
              memory.metadata = {};
            }
            ...
            elizaLogger.error(`Fallback memory creation also failed: ${fallbackError.message}`);
          }
        }
