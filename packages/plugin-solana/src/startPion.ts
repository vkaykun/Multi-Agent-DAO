...
import './shared/fixes/runtime-injector.js'; // Import the injector early
...
async function startPion() {
    try {
        elizaLogger.info("Starting Pion (Proposal Agent)");
        ...
        // Initialize runtime fixup early
        elizaLogger.info("🧩 Initializing runtime fixes from startPion");
        ...
        // Add runtime fix application after solanaRuntime is created
        elizaLogger.info("Explicitly applying runtime fixes to the created Solana runtime");
        try {
            const directResult = applyRuntimeFixes(solanaRuntime);
            
            elizaLogger.info("Runtime fix application result:", { 
                directApply: directResult
            });
            
            // Also store in global for other modules to access
            (global as any).solanaRuntime = solanaRuntime;
            elizaLogger.info("Set global.solanaRuntime reference");
            
            // Call global hook if defined
            ...
            elizaLogger.error("Error applying runtime fixes to Solana runtime:", fixError);
        }
