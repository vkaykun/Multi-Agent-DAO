    ...
    implements IDatabaseCacheAdapter
{
    ...
    async getMemoriesWithPagination(params: {
        ...
        const timeClause = params.startTime && params.endTime ? 'AND createdAt BETWEEN ? AND ?' : '';
        ...
        const stmt = this.db.prepare(sql);
        const memories = stmt.all(...values) as Memory[];
        ...
        const hasMore = parsedMemories.length > limit;
        const items = parsedMemories.slice(0, limit);
